...
DEFAULT_TEMPLATE = "my_template.docx.docx"
REPORTS_DIR = "reports"
TEMP_DIR = "temp_audio"
...
def render_step_one():
    ...
    with st.form("interview_info_form"):
        ...
        with col_left:
            ...
            with time_col2:
                m = st.selectbox("分钟", [f"{i:02d}" for i in range(0, 60, 10)], index=0)
        ...
        with btn_col:
            submitted = st.form_submit_button("🚀 下一步：上传文档", use_container_width=True)
...
def render_step_two():
    ...
    with col_right:
        ...
        # 尝试从上传的文件中提取文本
        extracted_transcript = st.session_state.transcription_text if st.session_state.transcription_text else st.session_state.transcript_text_cached
        ...
        if not st.session_state.config.get("api_key"):
            ...
            try:
                ...
                st.session_state.interview_result_summary = {
                    ...
                    "PreScreeningResult": interview_data.get("PreScreeningResult", "未知结论"),
    ...
    if st.session_state.last_report:
        ...
        with col_down2:
            if st.session_state.interview_result_summary:
                summary_docx = generate_summary_docx(st.session_state.interview_result_summary)
                st.download_button(
                    label="⬇️ 单独导出面试结果总览表",
                    data=summary_docx,
